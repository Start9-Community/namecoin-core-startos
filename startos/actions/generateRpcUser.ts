...
export const generateRpcUser = sdk.Action.withInput(
  ...
  async ({ effects }) => ({
    ...
    description: i18n(
      'Generate RPC User Credentials for remote connections i.e. Sparrow. rpcauth.py will randomly generate a secure password. The username and hashed password will be persisted in Namecoin.conf',
    ),
    warning: null,
    allowedStatuses: 'any',
  ...
  async ({ effects, input }) => {
    ...
    if (res.exitCode === 0 && typeof res.stdout === 'string') {
      ...
      return {
        ...
        message: i18n(
          'RPC password created for ${username}. Store this password in a secure place. If lost, a new RPC user will need to be created as Namecoin.conf only stores a hash of the password',
