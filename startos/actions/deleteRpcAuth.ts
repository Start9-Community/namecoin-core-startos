...
export const deleteRpcAuth = sdk.Action.withInput(
  ...
  async ({ effects }) => {
    ...
    return {
      ...
      description: i18n(
        'Delete RPC users from Namecoin.conf. You may want to run this action if the RPC Auth entry is no longer needed or if the password is lost.',
      ),
      warning: null,
      allowedStatuses: 'any',
  ...
  async ({ effects, input }) => {
    ...
    await namecoinConfFile.merge(effects, { raw: { rpcauth: filtered } })
  },
)
