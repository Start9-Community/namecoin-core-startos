...
export const reindexChainstate = sdk.Action.withoutInput(
  ...
  async ({ effects }) => ({
    ...
    allowedStatuses: 'any',
    group: i18n('Reindex'),
    visibility: (await namecoinConfFile.read().const(effects))?.prune
      ? 'hidden'
      : 'enabled',
  }),
  ...
  async ({ effects }) => {
    ...
    const status = await sdk.getStatus(effects, { packageId: 'namecoind' }).once()
    ...
    if (status?.desired.main === 'running') {
      ...
      return {
        version: '1',
        title: i18n('Success'),
        message: i18n('Restarting namecoind with -reindex-chainstate argument'),
        result: null,
      }
    }
    ...
    return {
      ...
      title: i18n('Success'),
      message: i18n(
        'The next time namecoind is started it will be run with the -reindex-chainstate argument',
      ),
      result: null,
    }
  },
