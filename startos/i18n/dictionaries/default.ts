...
const dict = {
  // main.ts
  'The Namecoin RPC Interface is ready': 1,
  'The Namecoin RPC Interface is not ready': 2,
  'Blockchain Sync': 3,
  'Namecoin is fully synced': 4,
  'Namecoin is starting…': 5,
  'Syncing blocks...${percentage}%': 6,
  'Connecting to peers and downloading headers…': 2200,
  'Syncing headers… ${headers} downloaded': 2201,
  'Syncing blocks... ${blocks} / ${headers} (${percentage}%)': 2202,
  'BIP9 Status': 2300,
  'BIP9 Bit': 2301,
  'BIP9 Start Time': 2302,
  'BIP9 Timeout': 2303,
  'BIP9 Since': 2304,
  'BIP9 Period': 2305,
  'BIP9 Threshold': 2306,
  ...
  'BIP9 Possible': 2309,
  'RPC Proxy': 7,
  'The Namecoin RPC Proxy is ready': 8,
  'The Namecoin RPC Proxy is not ready': 9,

  // interfaces.ts
  'RPC Interface': 100,
  'Listens for JSON-RPC commands': 101,
  'Peer Interface': 102,
  'Listens for incoming connections from peers on the namecoin network': 103,
  'ZeroMQ Interface': 104,
  'I2P Daemon Console': 105,
  'Interface to access the embedded I2P daemon console': 106,
  'Streams real-time Namecoin block and transaction notifications (hashes and raw data)': 107,
  ...
  'Delete RPC users from Namecoin.conf. You may want to run this action if the RPC Auth entry is no longer needed or if the password is lost.': 402,
  ...
  Reindex: 503,
  'Restarting namecoind with -reindex argument': 504,
  'The next time namecoind is started it will be run with the -reindex argument': 505,
  ...
  'Restarting namecoind with -reindex-chainstate argument': 603,
  'The next time namecoind is started it will be run with the -reindex-chainstate argument': 604,
  ...
  "The Transaction Index will be rebuilt once Namecoin Core is started again, unless 'Coinstats Index' is disabled in the config settings. Please don't do this unless you fully understand what you are doing.": 802,
  ...
  "The Coinstats Index will be rebuilt once Namecoin Core is started again, unless 'Transaction Index' is disabled in the config settings. Please don't do this unless you fully understand what you are doing.": 902,
  'Successfully deleted coinstats index': 903,

  // actions/assumeutxo.ts
  'UTXO Snapshot URL': 1000,
  'URL of UTXO Snapshot to bootstrap namecoin': 1001,
  'Download UTXO Snapshot (assumeutxo)': 1002,
  'assumeutxo is a feature that allows fast bootstrapping of a validating namecoind instance. It may take some additional time for any blocks between the snapshot blockheight and the tip to be downloaded and validated. While the snapshot is in use the IBD will continue in the background until it validates up to the snapshot blockheight': 1003,
  ...
  'Network and other runtime information about this Namecoin node': 1101,
  'Node Runtime Info': 1102,
  Connections: 1103,
  'The number of peers connected (inbound and outbound)': 1104,
  'IPC Socket Path': 1105,
  'Unix socket path for IPC communication with Namecoin Core. Other services can bind to this socket in their Docker configuration.': 1106,
  ...
  Connect: 1246,
  'Connect Nodes': 1247,
  'Add addresses of nodes for Namecoin to EXCLUSIVELY connect to.': 1248,
  ...
  'Add Node': 1250,
  'Add Nodes': 1251,
  'Add addresses of nodes for Namecoin to connect with in addition to default nodes.': 1252,
  'Blocks Only': 1258,
  ...
  unlimited: 1257,
  'Peer Settings': 1253,
  'Edit the Peer settings in namecoin.conf': 1254,
  ...
  'Edit the Mempool settings in namecoin.conf': 1515,
  ...
  requests: 1607,
  'RPC Settings': 1608,
  'Edit the RPC settings in namecoin.conf': 1609,
  ...
  'The fee rate (in NMC/kB) that indicates your tolerance for discarding change by adding it to the fee.': 1716,
  'NMC/kB': 1717,
  Pruning: 1718,
  ...
  'Requires Tor or I2P to be active. Namecoin Core will refuse to start if neither is available.': 2004,
  'Database Batch': 1724,
  ...
  'Enable IPC': 1736,
  'Enable inter-process communication (IPC) via Unix socket. This allows other services to communicate with Namecoin Core using a high-performance local socket connection. The socket path will be displayed in Runtime Information.': 1737,
  ...
  'Other Settings': 1739,
  'Edit more values in namecoin.conf': 1740,
  ...
  'Prepare Namecoin Core to back a local ElectrumX server: generate an RPC user for ElectrumX, disable pruning (prune=0), and enable the transaction index (txindex=true). The generated password is shown once — copy it into ElectrumX immediately. If pruning was previously enabled, a full blockchain reindex will be scheduled on next start.': 2401,
  'Disabling pruning requires a full (non-pruned) chain. If pruning was previously enabled, Namecoin Core will reindex from genesis on next start — this can take hours and needs significantly more disk space (~10–15 GB chain + ~5 GB ElectrumX index).': 2402,
  'RPC username ElectrumX will use to connect to Namecoin Core. Defaults to "electrumx".': 2403,
  'Namecoin Core configured for ElectrumX': 2404,
  'RPC user "${username}" created, pruning disabled, transaction index enabled. Copy the password below into ElectrumX\'s "Namecoin Core Connection" action. The password is shown ONCE — if lost you will need to create a new RPC user. ${reindexNote}': 2405,
  'Pruning was previously enabled; Namecoin Core will perform a full blockchain reindex on next start.': 2406,
