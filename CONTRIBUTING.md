...
make      # build the .s9pk for the supported arches (x86_64, aarch64)
```

The `namecoind` image is built from source by `Dockerfile` (CMake build of the pinned `nc<VERSION>` tag), so the first build compiles Namecoin Core and takes a while. For a complete list of build options, see [Makefile](https://docs.start9.com/packaging/makefile.html).
